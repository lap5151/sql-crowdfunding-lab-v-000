...
"SELECT users.name, users.age, SUM(pledges.amount) AS 'Pledge Total' FROM users
...
"SELECT projects.title, projects.funding_goal,SUM(pledges.amount), SUM(pledges.amount) - projects.funding_goal AS 'Overage' FROM projects
INNER JOIN pledges ON pledges.project_id = projects.id
GROUP BY projects.title
